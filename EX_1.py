...
while running:
    ...
    if action is not None:
        s_decoded = decode_state(obs)
        feats = extract_features(s_decoded)
        feats["action"] = action
        feats["map_name"] = map_file   # 👈 guardamos también el nombre del mapa
        data.append(feats)
...
if len(data) > 0:
    ...
    dataset_name = f"dataset_manual_{map_file.replace('.txt','')}.csv"
    dataset_path = os.path.join("data/manual", dataset_name)
